...
public class PlayerScoreboard {
    ...
    public PlayerScoreboard(WolfGame wolfGame, Player player){
        this.player = player;
    ...
    public void setScores(){
        stateScore = "   └ "+ wolfGame.getWareWolfPlayers().get(player).getRole().getRoleName();
        timeScore = "   └ "+ wolfGame.getTimeZone().getTime()+"秒";

        obj.getScore("+ あなたの役職").setScore(-1);
        obj.getScore(stateScore).setScore(-2);
        obj.getScore(ChatColor.GRAY+"+ 墓場 - "+ wolfGame.getDeadPlayers().size()+"人").setScore(-99);
